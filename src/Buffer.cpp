...
template<typename T>
Buffer<T>::Buffer(const size_t size): new_data(true), age(0), size(size){
...
template<typename T>
void Buffer<T>::write_offset(const T buf[], const size_t n, const size_t gap, const size_t offset){
	auto lock = this->lock();
